...
- `sync-server-1` (or similar) — the sync API on port 3003
- `postgres-1` — PostgreSQL on port 5432
- `grafana-1` — Grafana on port 3004
...
```powershell
docker logs sync-server-1 --tail 50
```
...
```powershell
docker exec postgres-1 psql -U postgres -d dora_metrics -c "SELECT id, status, records_synced, error_message FROM sync_jobs ORDER BY id DESC LIMIT 3;"
```
...
```powershell
docker exec postgres-1 psql -U postgres -d dora_metrics -c "TRUNCATE sync_state;"
```
