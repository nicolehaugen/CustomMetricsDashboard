...
- **Enterprise slug** *(required)* — the sync server (`src/config.ts`)
...
If `.env` already exists and has a non-placeholder `GITHUB_TOKEN`, warn the
user that the existing token will be overwritten and ask for confirmation
before proceeding.

## 2. Create or update `.env`

- If `.env` does not exist, copy `.env.example` to `.env` first.
- Set these variables with the user-provided values:
  ...
  GITHUB_ORG=<org>
  GITHUB_REPO=<repo>
  GITHUB_ENTERPRISE=<enterprise>   # required — src/config.ts throws if unset
  ```
...
- After changing the token, restart the sync server to pick up new values:
  `docker compose restart sync-server` from repo root.
- Required PAT scopes: `repo`, `read:org`, `admin:org`, `actions`.
