...
describe.each(ALL_DASHBOARDS)('%s — common structure', (filename) => {
  ...
  it('panels include descriptions (strict for core dashboards)', () => {
    dashboard = loadDashboard(filename);
    if (filename.endsWith('-edu.json')) {
      expect(dashboard.panels[0].description, `${filename} banner panel should include a description`).toBeTruthy();
      const hasAnyDescription = dashboard.panels.some((panel: any) => Boolean(panel.description));
      expect(hasAnyDescription, `${filename} should include at least one panel description`).toBe(true);
      return;
    }
