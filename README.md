...
2. **Check PostgreSQL has data:**
   ```bash
   docker exec postgres-1 psql -U postgres -d metrics -c "SELECT COUNT(*) FROM sync_jobs;"
   ```
   If returns `0`, trigger a sync: `curl -X POST http://localhost:3005/api/sync`

3. **Check Copilot metrics were fetched:**
   ```bash
   docker exec postgres-1 psql -U postgres -d metrics -c "SELECT COUNT(*) FROM copilot_enterprise_daily_summary;"
   ```
   If returns `0`, check sync server logs: `docker logs sync-server-1 | grep -i copilot`
...
- Check sync logs: `docker logs sync-server-1 | tail -50`
